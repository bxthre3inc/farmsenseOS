...
- **Vertical Field Anchors (VFA) [Level 1 - Advanced Peer Node]:** The 48-inch deep-profile central ground-truth soil node. Utilizes **HDPE SDR9** for maximum chemical resistance and impact strength in high-alkali soil. Communicates via 128-bit encrypted 900MHz FHSS directly to the elevated PMT.
- **Lateral Root-Zone Scout (LRZ) [Level 1 - Spatial Mapper]:** Mass-produced "dumb nodes" deployed at a 1:15-acre density. Utilizes identical **900MHz FHSS AES-128** networking to match VFA abilities, providing uniform field-layer state data to the PMT. Enclosures feature 50mm non-contact capacitive telemetry fields.
- **Pressure & Flow Anchor (PFA) [Sentry of the Source]:** Mounts at the wellhead, monitoring vibration torque ripple, cavitation, and bearing wear via 400A CT Clamps. Runs NXP Cortex-M7 edge processors preparing to process Current Harmonic Analysis signatures. Uses **900MHz FHSS** links to bounce data to the elevated PMT, ensuring absolute penetration through canopy interference.
- **Pivot Motion Trackers (PMT) [Level 1.5 - Field Hub & Edge-EBK Engine]:** The command center and Nervous System of the field, mounted 10-15 feet high on the pivot span.
  - **RF Umbrella:** Receives **900MHz FHSS AES-128 chirps** from the VFA, LRZs, and PFA, bundling the entire field state into a single ~187-byte AES-256 payload and beaming it to the DHU via **900MHz LoRaWAN & LTE-M/NB-IoT failover**.
