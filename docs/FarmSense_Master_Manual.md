...
```mermaid
graph TD
    A[Sensors: LRZ1/LRZ2, VFA, PFA] -->|900MHz CSS LoRa| B(PMT Field Hub)
    B -->|5GHz Sector Mesh| C{District Hub - DHU}
    C -->|Fiber / Starlink| D[Regional Superstation - RSS]
    D -->|Sovereign Data Vault| E[Zo Scientist Engine]
    
    subgraph Edge_Layer [Field Level]
        B
    end
    
    subgraph Coordination_Layer [District Level]
        C
    end
    
    subgraph Intelligence_Layer [Territory Level]
        D
        E
    end
```
